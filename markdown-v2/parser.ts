import type { Token, DirectiveToken, HtmlBlockToken } from './types';
  ...
  while (i < lines.length) {
    ...
    if (tagStartMatch) {
      ...
      if (openTagMatch) {
        ...
        // Compress newlines in attributes so the React renderer doesn't break
...
function splitSlots(rawContent: string): Record<string, string> {
  ...
  for (const line of lines) {
    ...
    // Track directive nesting depth (::: opening / ::: closing)
    if (trimmed.startsWith(':::')) {
      const rest = trimmed.slice(3).trim();
      if (rest === '') {
        // Bare `:::` → closing marker
    ...
    // Only split on #slotname when at the top level (not inside a nested directive)
    const slotMatch = trimmed.match(/^#([\w-]+)$/);
    if (slotMatch && nestingDepth === 0) {
      // Flush buffer into current slot
