...
| `headerButtons` | `EditorToolbarButton[]` | — | Botones adicionales en la toolbar del editor |
...
Para añadir botones personalizados a la toolbar del editor, pasa un array de
objetos `EditorToolbarButton` al prop `headerButtons`:
...
|----------|------|-------------|
| `icon` | `string` | Material icon name (e.g. `'download'`, `'share'`, `'code'`) |
| `label` | `string` | Button label text |
| `onClick` | `() => void` | Click handler (**mutually exclusive** with `url`) |
| `url` | `string` | URL to open on click (**mutually exclusive** with `onClick`) |
| `target` | `string` | Link target (default: `'_blank'` when `url` is used) |

> Los botones aparecen a la derecha de "Guía" y "Configurar" en la toolbar,
> separados por divisores automáticos. Se pueden añadir tantos como se necesite.
